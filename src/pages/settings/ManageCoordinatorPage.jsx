...
import { createCoordinator, deactivateCoordinator, getCoordinators, getUsersForRoleAssignment, setUserRole } from "../../api/roles";

const placeholderAvatar =
  "https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png";

export default function ManageCoordinatorPage() {
  ...
  const hasCoordinator = rows.length > 0;
  ...
  const deactivateMutation = useMutation({
    mutationFn: deactivateCoordinator,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["coordinators"] });
      queryClient.invalidateQueries({ queryKey: ["administrators"] });
      queryClient.invalidateQueries({ queryKey: ["usersForRoles"] });
            ...
            <div className="card border-0 shadow-sm rounded-4 mb-3">
              <div className="card-header bg-white border-bottom py-3">
                ...
                <p className="text-muted small mb-0 mt-1">
                  Select an existing user and make them a Coordinator.
                </p>
              </div>
              <div className="card-body">
                {hasCoordinator && (
                  <div className="alert alert-warning py-2">
                    Only one Coordinator is allowed. Remove the current Coordinator to assign a new one.
                  </div>
                )}
                      ...
                      <div key={u.id} className="d-flex align-items-center gap-2 p-2 border rounded-4">
                        ...
                        <button
                          ...
                          disabled={
                            ...
                            !!u.isCoordinator ||
                            !!u.isSubAdmin ||
                            (hasCoordinator && !u.isCoordinator)
                          }
                          onClick={() => assignMutation.mutate({ userId: u.id, role: "coordinator", enabled: true })}
                        >
                          {u.isCoordinator ? "Assigned" : "Make Coordinator"}
                        </button>
                      </div>
                    ))}
                  </div>
              ...
              <div className="card-body p-0">
                {isLoading ? (
                  ...
                  <div className="text-center py-5 text-danger">
                    ...
                    <div>{error?.message || "Failed to load Coordinators"}</div>
                  </div>
                ) : rows.length === 0 ? (
                  <div className="text-center py-5">
                    <p className="text-muted mb-0">No Coordinators yet.</p>
                  </div>
                ) : (
